...
public class StudentsDao {
	...
	public static int doUpdateNowRecord(String originalusername, String newuserid, String newpassword, String newname, String newemail) {
	    int status = 0;
	    try {
	        Connection con = Provider.getOracleConnection();
	        String sql = "update studenttable set userid=?, password=?, name=?, emailid=? where userid=?";
	        PreparedStatement pst = con.prepareStatement(sql);
	        pst.setString(1, newuserid);
	        pst.setString(2, newpassword);
	        pst.setString(3, newname);
	        pst.setString(4, newemail); // Updated parameter position for emailid
	        pst.setString(5, originalusername);
	        int val = pst.executeUpdate();
	        if (val > 0) {
	            status = 1;
	        } else {
